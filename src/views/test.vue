...
const getInput = (data) => {
  if(product.OrderList.length == 0){
    let product_n = product.allInput
    product_n.push(data);
...
const getOrder = async(data) => {
  if (product.OrderList.length === 0) {
    ...
    console.log('product.OrderList', product.OrderList);
...
async function moneyTurn(total) {
  ...
  product.returnMoney = bank + (bankNote.thousand * 1000);
  product.bankNote = bankNote;
  product.Note = bankNote;
  let inputOne = product.inputOne - bankNote.one;
  let inputFive = product.inputFive - bankNote.five;
  let inputTen = product.inputTen - bankNote.ten;
  let inputTwenty = product.inputTwenty - bankNote.twenty;
  let inputFifty = product.inputFifty - bankNote.fifty;
  let inputHundred = product.inputHundred - bankNote.hundred;
  let inputFiveHundred = product.inputFiveHundred - bankNote.fivehundred;
  let inputThousand = product.inputThousand - bankNote.thousand;
  ...
  const money = {
    ...
    twenty: inputTwenty,
    fifty: inputFifty,
    hundred: inputHundred,
    fivehundred: inputFiveHundred,
    thousand: inputThousand
  }
  console.log('>>>', money);
  // res = await moneyStore.updateMoney(money)
